...
import com.jmc.scm.common.CustomExcelReportBuilder;
 ...
 * 
 */
@Component(CustomExportPR.BEAN_ID)
public class CustomExportPR {

	public static final String BEAN_ID = "customExportPR";
	...
	@Autowired
	@Qualifier(CustomExcelReportBuilder.BEAN_ID)
	public CustomExcelReportBuilder excelReportBuilder;
	...
	@Expose
	public Map<String, String> generateReportFile(Map<String, Object> parameter)
			throws Exception {
		...
		String location = ExportUtils.getFileStorePath() + id + "_" + fileName
				+ "." + extension;
		Map<String, Object> titleInfos = (Map<String, Object>) parameter
				.get("titleInfos");
		List<Map<String, Object>> reportInfos = (List<Map<String, Object>>) parameter
				.get("reportInfos");
		ReportTitle reportTitle = excelReportModelGenerater
				.generateReportTitleModel(titleInfos);
		if (FileExtension.xls.equals(extension)
				|| FileExtension.xlsx.equals(extension)) {
			this.generateExcelFile(reportTitle, reportInfos, rowSpace,
					fileName, location, interceptorName);
		} else if (FileExtension.pdf.equals(extension)) {
			this.generatePdfFile(reportTitle, reportInfos, rowSpace, location,
					interceptorName);
		} else {
			this.generateOtherFile(extension, reportTitle, reportInfos,
					rowSpace, location, interceptorName);
		}
	...
	@SuppressWarnings("unchecked")
	private void generatePdfFile(ReportTitle reportTitle,
			List<Map<String, Object>> reportInfos, int rowSpace,
			String fileName, String interceptorName) throws Exception {
		...
		try {
			for (Map<String, Object> map : reportInfos) {
				...
				if (SupportWidget.grid.name().equals(type)) {
					Map<String, Object> grid = (Map<String, Object>) map
							.get("grid");
					pdfReportBuilder.addGridToDocument(doc, reportTitle,
							pdfReportModelGenerater.generateReportGridModel(
									grid, interceptorName));
					pdfReportBuilder.addNewline(doc, rowSpace);
				} else if (SupportWidget.form.name().equals(type)) {
					Map<String, Object> form = (Map<String, Object>) map
							.get("form");
					ReportForm reportFormModel = pdfReportModelGenerater
							.generateReportFormModel(form, interceptorName);
					pdfReportBuilder.addFormToDocument(doc, reportTitle,
							reportFormModel);
					pdfReportBuilder.addNewline(doc, rowSpace);
				}
			}
	...
	@SuppressWarnings("unchecked")
	private void generateExcelFile(ReportTitle reportTitle,
			List<Map<String, Object>> reportInfos, int rowSpace,
			String fileName, String location, String interceptorName)
			throws Exception {
		...
		if (location.endsWith(FileExtension.xls)) {
			...
			workbook = excelReportBuilder
					.createWorkBook2007(rowAccessWindowSize);
		}
		...
		for (Map<String, Object> map : reportInfos) {
			...
			if (SupportWidget.grid.name().equals(type)) {
				Map<String, Object> grid = (Map<String, Object>) map
						.get("grid");
				ReportGrid reportGridModel = excelReportModelGenerater
						.generateReportGridModel(grid, interceptorName);
				if (index == 0) {
					List<ReportGridHeader> bottomColumnHeaderModelList = new ArrayList<ReportGridHeader>();
					excelReportBuilder.calculateBottomColumnHeader(
							reportGridModel.getGridHeaderModelList(),
							bottomColumnHeaderModelList);
					int bottomColumnHeaderCount = bottomColumnHeaderModelList
							.size();
					reportGridModel.setColumnCount(bottomColumnHeaderCount);
					nextRow = excelReportBuilder.addTitleToSheet(reportTitle,
							sheet, bottomColumnHeaderCount - 1);
				}
				nextRow = excelReportBuilder.addGridToSheet(reportGridModel,
						sheet, nextRow);
				...
				Map<String, Object> form = (Map<String, Object>) map
						.get("form");
				ReportForm reportFormModel = excelReportModelGenerater
						.generateReportFormModel(form, interceptorName);
				if (index == 0) {
				...
				nextRow = excelReportBuilder.addFormToSheet(reportFormModel,
						sheet, nextRow);
				nextRow = nextRow + rowSpace;
			}
			index++;
	...
	@SuppressWarnings("unchecked")
	private void generateOtherFile(String extension, ReportTitle reportTitle,
			List<Map<String, Object>> reportInfos, int rowSpace,
			String fileName, String interceptorName) throws Exception {
		FileOutputStream out = new FileOutputStream(fileName);
		try {
			for (Map<String, Object> map : reportInfos) {
				...
				if (SupportWidget.grid.name().equals(type)) {
					Map<String, Object> grid = (Map<String, Object>) map
							.get("grid");
					ReportGrid report = commonReportGenerater
							.generateReportGridModel(grid, interceptorName);
	...
	public void afterPropertiesSet() throws Exception {
		...
		builders = applicationContext.getBeansOfType(ReportBuilder.class)
				.values();
	}
