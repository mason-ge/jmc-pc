...
public class QRCodeBean extends AbstractBarcodeBean {
	...
	/**
	 * Sets the message encoding. The value must conform to one of Java's
	 * encodings and have a mapping in the ECI registry.
	 * 
	 * @param encoding
	 ...
	 * 
	 * @param minSize
	 *            the minimum size (in pixels), or null for no constraint
	 */
	...
	/**
	 * Returns the minimum symbol size that is to be produced. If the method
	 * returns null, there's no constraint on the symbol size.
	 * 
	 * @return the minimum symbol size (in pixels), or null if there's no size
	 *         constraint
	 */
	...
	/**
	 * Returns the maximum symbol size that is to be produced. If the method
	 * returns null, there's no constraint on the symbol size.
	 * 
	 * @return the maximum symbol size (in pixels), or null if there's no size
	 *         constraint
	 */
	...
	public void generateBarcode(CanvasProvider canvas, String msg) {
		...
		impl.setImageWidth(this.getImageWidth());
		impl.setImageHeight(this.getImageHeight());
	...
	public BarcodeDimension calcDimensions(String msg) {
		...
		return new BarcodeDimension(width, height, width + (2 * qzh), height
				+ (2 * qzv), qzh, qzv);
	}

	private void checkSizeConstraints(int width, int height) {
		// Note: we're only checking the constraints, we can't currently
		// influence ZXing's encoder.
		if (this.minSize != null) {
			if (width < this.minSize.width || height < this.minSize.height) {
				throw new IllegalArgumentException(
						"The given message would result in a smaller symbol than required."
								+ " Requested minimum: " + this.minSize.width
								+ " x " + this.minSize.height + ", effective: "
								+ width + " x " + height);
			}
		}
		if (this.maxSize != null) {
			if (width > this.maxSize.width || height > this.maxSize.height) {
				throw new IllegalArgumentException(
						"The given message would result in a larger symbol than required."
								+ " Requested maximum: " + this.maxSize.width
								+ " x " + this.maxSize.height + ", effective: "
								+ width + " x " + height);
			}
		}
	}
