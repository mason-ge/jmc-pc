...
!function(dsBase, dsItem, uaSaveOrUpdate) {
	...
	flushCode = baseData.get('supCode');
	// 保存前校验
		...
		if (re) {
			...
			if (action == 'mod' || action == 'new') {
				if (result == '10') {
					...
					view.set('#supCodeForm.readOnly', true);
...
!function(self, arg, dsItem, dsBase, ajCheckSupCode) {
	...
	if (newValue!== null && newValue!== '') {
		if (property == 'supCode') {
			ajCheckSupCode.set('parameter', newValue);
			ajCheckSupCode.execute(function(re) {
				if (!re) {
					dorado.MessageBox.alert('存在相同的供应商编码！');
					currEntity.setMessages(property, '存在相同的供应商编码！');
				}
			});
		}
	}
